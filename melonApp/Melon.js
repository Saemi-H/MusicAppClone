...
const Melon = () => { 
    ...
    const [findData, setFindData] = useState(data[0])
    ...
    const clicked=(id)=>{
        setOpen(true)
        const newData=data.find(item=> item.rank===id)
        setFindData(newData)
    }
    const close=()=>{
        setOpen(false)
    ...
    return (
        <div className="Melon">
            ...
            <MelonList 
                ...
                clicked={clicked}
            />
           { open && <MelonModal findData={findData} close={close}/>}
        </div>
    );
};
